...
import { setNotation, totalReps } from '../utils'
...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-gray-900 border border-white/10 rounded-xl px-3 py-2.5 shadow-xl">
      ...
      <p className="text-white font-bold text-sm">{d.totalReps} reps</p>
    ...
    return (allData.sessions || [])
      ...
      .map(s => {
        ...
        return {
          ...
          notation: setNotation(ex.sets),
          sets: ex.sets.length,
        }
      })
  }, [exerciseName, allData])

  const last = chartData[chartData.length - 1]
  const prev = chartData[chartData.length - 2]
  const trend = chartData.length >= 2 ? last.totalReps - prev.totalReps : null
                ...
                <div className="flex items-center gap-2 mb-5">
                  {trend > 0 ? (
                    <span className="flex items-center gap-1.5 text-green-400 text-sm font-medium">
                      <TrendingUp size={16} /> +{trend} reps vs previous session
                    </span>
                  ) : trend < 0 ? (
                    <span className="flex items-center gap-1.5 text-red-400 text-sm font-medium">
                      <TrendingDown size={16} /> {trend} reps vs previous session
                    </span>
                  ) : (
              ...
              <div className="space-y-2">
                ...
                {[...chartData].reverse().map((d, i) => (
                  <div key={i} className="flex items-center justify-between py-1">
                    <span className="text-gray-500 text-xs w-28">{d.fullDate}</span>
                    <span className="text-cyan-400 font-mono text-sm font-semibold flex-1 text-center">
                      {d.notation}
                    </span>
  ...
  const exercises = useMemo(() => {
    ...
    ;(allData.sessions || []).forEach(s => {
      ;(s.exercises || []).forEach(ex => {
        if (!map.has(ex.name)) {
          map.set(ex.name, { name: ex.name, category: ex.category || 'Custom', count: 0, lastReps: 0, lastNotation: '—' })
        }
        const entry = map.get(ex.name)
        entry.count++
        entry.lastReps = totalReps(ex.sets)
        entry.lastNotation = setNotation(ex.sets)
      })
    })
